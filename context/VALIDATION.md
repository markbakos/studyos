...
| GitHub Pages                              | Deployment workflow configured, not deployed validated            | The workflow builds with `/studyos/` as the Vite base and publishes `dist` through GitHub Pages. A successful GitHub Actions run and live URL check remain open.                                                                                                             |
| Domain services                           | Partial automated runtime evidence                                | 2026-09-08: all 15 tests in `tests/domain.test.ts` pass, covering planning, hierarchy, learning, sessions, prompt generation, pack/backup validation, and injected transaction rollback. The backup fixture does not contain stored blobs despite its test title.         |
| External AI creation workflow             | Implemented with automated prompt evidence, not browser validated | 2026-09-08: task-specific prompt generation is covered in `tests/domain.test.ts`; the UI includes the exact schema and reuses Study Pack validation/preview/import for pasted JSON. Clipboard, external-model output quality, and browser interaction remain unvalidated. |
...
Runtime commands: `node --import tsx tests/domain.test.ts` and `node --import tsx tests/planner.test.ts`. The `npm test` CLI was blocked by sandbox IPC permissions (`listen EPERM`); direct Node execution uses the installed tsx loader and runs all 20 named tests without the CLI socket.
