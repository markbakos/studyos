...
description: "Build, refactor, and review React and TypeScript applications using feature-oriented architecture, strict boundary typing, small composable components, stateless and context-driven compound UI systems, predictable list rendering, deliberate state ownership, and maintainable interaction patterns. Use for React/TypeScript components, feature organization, lists and tables, filters, editors, routing, state APIs, or frontend code-quality reviews."
---
...
### Namespace stateless compound APIs

Do not equate namespaced syntax with Context. When parts need only their own props and children:

- Export ordinary components through a narrow module and import them as a namespace.
- Keep domain content explicit in the composing parent instead of hiding small static structures in configuration arrays.
- Group a coherent component family in its own folder with a deliberate public entry point.
- Add Context later only when descendants must coordinate shared state, actions, or metadata.

```tsx
import * as Nav from './elements'

<Nav.Root>
  <Nav.Section title="Portfolio">
    <Nav.Item to="/brands"><BrandIcon /><span>Brands</span></Nav.Item>
  </Nav.Section>
</Nav.Root>
```
